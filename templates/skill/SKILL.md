...
name: {{NAME}}
description: >
  <!-- FILL: Rewrite as trigger conditions — ≥ 2 quoted user phrases + concrete activation conditions (not passive summary). Copy the final block byte-for-byte into .cursor/skills/{{NAME}}/SKILL.md — smoke-test enforces equality. -->
...
<!-- <always-applicable> / <task-routing> tags below are load-bearing:
     XML blocks survive context compression better than plain headings.
     Details: references/thin-shells.md § XML-Tag Injection. -->
...
<!-- FILL: 3–7 project-specific principles. Each MUST end with a ✓ Check: sentence —
     a concrete post-execution question. Declarative-only principles get "remembered"
     before acting but have no hook to trigger verification. Examples below are
     realistic shapes, not your actual rules — replace every one and delete these lines. -->
