...
1. **Define the Done Contract** — name owned files/modules, the observable outcome, the cheapest sufficient validation set, and concrete signals that would expand context or escalate validation. For business-bearing work, record `business-model impact: unchanged / proposed change / unknown`; a proposed type/flow/state/boundary/invariant change requires an approved Plan.
...
8. **Validate behavior** by running the frozen validation set. Escalate only when a named risk signal fired; once the set is green, do not invent new coverage, runtime startup, or broader suites.
...
Done Contract, invariant, ownership/provenance, call chain, all semantic paths, fan-out targets, sync, drift, frozen validation, escalation signals, and Task Closure are all accounted for.
